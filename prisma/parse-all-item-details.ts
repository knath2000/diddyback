...
async function extractInfo(html: string) {
  ...
  if (!description) description = $('.product-caption').text().trim();
  ...
  $('.product-thumb__slider a').each((_, a) => {
    ...
    if (url) images.push(`https://www.supremecommunity.com${url}`);
  });
