...
let getDroplist: undefined | ((args: any) => Promise<any[]>);
try {
...
async function fetchViaApi(): Promise<ScrapedItem[] | null> {
  try {
    if (!getDroplist) return null;
    const apiRes: { name: string; image: string }[] = await getDroplist({
      season: 'spring-summer2025',
      date: '2025-07-03'
    });
    if (!apiRes?.length) return null;
...
async function fetchItems(): Promise<ScrapedItem[]> {
  ...
  const url =
    'https://www.supremecommunity.com/season/spring-summer2025/droplist/2025-07-03/';
  ...
  let contentSource: Page | Frame = page;
  let html = '';
...
async function upsertItems(items: ScrapedItem[]) {
  if (!items.length) {
